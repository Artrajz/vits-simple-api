...
def g2p(text, pinyin_g2pw):
    pattern = r"(?<=[{0}])\s*".format("".join(punctuation))
    sentences = [i for i in re.split(pattern, text) if i.strip() != ""]
    phones, word2ph = _g2p(sentences, pinyin_g2pw)
...
def _g2p(segments, pinyin_g2pw=None):
    phones_list = []
    word2ph = []
    for seg in segments:
        pinyins = []
        ...
        if not is_g2pw:
            ...
            # g2pw采用整句推理
            pinyins = pinyin_g2pw.lazy_pinyin(seg, neutral_tone_with_five=True, style=Style.TONE3)
